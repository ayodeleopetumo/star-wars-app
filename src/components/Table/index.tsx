...
const Table: React.FC<Prop> = ({movieCharacterInfo}) => {
  const selectGenderConstants = [{value: 'all', label: 'All'}, {value: 'male', label: 'Male'}, {value: 'female', label: 'Female'}, {value: 'hermaphrodite', label: 'Hermaphrodite'}, {value: 'none', label: 'Non Human'}];
  const characters = movieCharacterInfo?.characters!;
  const movieInfo = movieCharacterInfo?.movie!;

  const createMarkup = () => {
    return { __html: `<marquee direction='up' scrollamount='2' class='movies-list__opening-crawl-text'>${movieInfo.opening_crawl}</marquee>`
    };
  }

  const getCharacterCount = (characters: People[]) => characters.length
  const getTotalHieght = (characters: People[]): string => {
    const heightInCentimeters = characters.filter(character => !isNaN(+character.height)).reduce((acc, curr) => acc + +curr.height, 0);
    ...
    <div className='movie-list__movie-character-info'>
      ...
      <div className='movies-list__gender-filter'>
        <select className='movies-list__gender-filter-select' onChange={evt => console.log(evt.currentTarget.value)}>
          { selectGenderConstants.map((gender, index) => <option key={index} value={gender.value}>{gender.label}</option>) }
        </select>
      </div>
